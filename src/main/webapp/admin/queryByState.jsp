...
<form action="<%=path%>/querySchedule" method="get">
    ...
    <table width="98%" border="0" cellpadding="2" cellspacing="1" align="center" style="margin-top:8px">
        <tr>
            <td>
                日程状态<select name="states" id="states" data-placeholder="日程状态">
                    <option value=""></option>
                    <option value="">全部</option>
                    <option <c:if test="${requestScope.status=='0'}">selected="selected"</c:if> value="0">未到期日程</option>
                    <option <c:if test="${requestScope.status=='1'}">selected="selected"</c:if> value="1">提醒日程</option>
                    <option <c:if test="${requestScope.status=='2'}">selected="selected"</c:if> value="2">已完成日程</option>
                    <option <c:if test="${requestScope.status=='3'}">selected="selected"</c:if> value="3">过期日程</option>
                </select>
