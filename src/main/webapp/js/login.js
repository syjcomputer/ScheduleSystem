...
$(function() {
    ...
    $("#userName").blur(function () {
        var account = $(this).val(); // this相当于当前input对象， val是value
        if (account == null || account.trim() == "") {
            return;
        }
        $.post("../validateUser", "account=" + account, function (data) {
            if (data == "1") {
                $("#inputEmail").val("");
                // $(".error-msg").first().css("display", "block"); // 设置第一个error-msg标签为block样式
            } else {
                pwd = data;
                loginTrue = true;
            }
